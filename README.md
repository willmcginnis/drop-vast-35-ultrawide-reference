...
| **Does DDC/CI work?** | **Barely.** It answers **exactly one** transaction per power event, then returns Null Messages until power-cycled. Established: process churn excluded by `selftest`, pacing excluded by `patience` out to 80 s gaps. Not a usable control path. |
